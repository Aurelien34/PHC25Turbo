...
GREETINGS_DATA_BIN=$(TO_COMPRESS_PATH)/greetingsdata.bin
GREETINGS_DATA_SOURCE=greetingsdata.special
...
$(GREETINGS_TEXT_BIN): $(GREETINGS_TEXT_SOURCE) $(TO_COMPRESS_PATH)
	$(AS) $< -Fbin -o $@ -quiet
$(GREETINGS_DATA_BIN): $(GREETINGS_DATA_SOURCE) $(TO_COMPRESS_PATH)
	$(AS) $< -Fbin -o $@ -quiet

$(GREETINGS_OBJ): $(GREETINGS_TEXT_BIN) $(GREETINGS_DATA_BIN)
...
$(COMPRESSED_FILES_INCLUDER): $(EXTRACTED_TO_COMPRESS) $(CIRCUITS_BIN) $(GS_IMAGES_RAW) $(COLOR_IMAGES_RAW) $(GREETINGS_TEXT_BIN) $(GREETINGS_DATA_BIN)

$(OBJ_PATH)/%.o: %.s $(INC) $(PRECOMP_PATH)
	$(AS) $(ASFLAGS) -L $(PRECOMP_PATH)/$<.txt -o $@ $< -DDEBUG=$(DEBUG)

$(RLH_COMPRESSOR_SOURCE_FILE) $(COMPRESSED_FILES_INCLUDER) $(COMPRESSED): $(TO_COMPRESS_ALL) $(COMPRESSION_PATH) $(TO_COMPRESS_PATH) $(COLOR_IMAGES_RAW) $(GS_IMAGES_RAW)
...
ifneq ($(wildcard $(SAVESTATE)),)
	...
	cd $(TO_COMPRESS_PATH) & $(foreach FILE,$(GREETINGS_DATA_BIN),del $(notdir $(FILE)) &)
